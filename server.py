...
def detection_thread(source):
    ...
    # Signal that models are loaded and ready
    with state_lock:
        detection_state['models_ready'] = True

    src = int(source) if str(source).isdigit() else source

    # ── FIX: Use DirectShow on Windows first (avoids MSMF conflicts) ─────────
    cap = cv2.VideoCapture(src, cv2.CAP_DSHOW)
    if not cap.isOpened():
        print(f"[WARN] DirectShow failed for source {src} — trying default backend ...")
    ...
    while True:
        ...
        # ── FIX: pass all 11 arguments to match draw_overlay signature ───────
